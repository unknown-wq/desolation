...
public class DesolationRegistries {
    ...
    public static <B extends Block> B register(String name, Function<BlockBehaviour.Properties, B> factory, BlockBehaviour.Properties properties) {
        ResourceKey<Block> key = ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath(Desolation.MOD_ID, name));
    ...
    public static <I extends Item> I register(String name, Function<Item.Properties, I> factory, Item.Properties properties) {
        ResourceKey<Item> key = ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(Desolation.MOD_ID, name));
    ...
    public static <T extends Feature<FC>, FC extends FeatureConfiguration> T register(String name, T feature) {
        return Registry.register(BuiltInRegistries.FEATURE, Identifier.fromNamespaceAndPath(Desolation.MOD_ID, name), feature);
    }

    /**
     * Creates and registers a configured feature (during datagen).  The configured feature will be created
     * with the provided feature config and registered to the provided context under the provided key.
     *
     * @param context A registration-capable abstraction of a registry of configured features
    ...
    public static <FC extends FeatureConfiguration, F extends Feature<FC>> void register(BootstrapContext<ConfiguredFeature<?, ?>> context, ResourceKey<ConfiguredFeature<?, ?>> key, F feature, FC config) {
        FeatureUtils.register(context, key, feature, config);
    }

    /**
     * Creates and registers a placed feature (during datagen).  The placed feature will be created with
     * the provided placement modifiers and registered to the provided context under the provided key.
     *
     * @param context A registration-capable abstraction of a registry of placed features
     ...
     */
    public static void register(BootstrapContext<PlacedFeature> context, ResourceKey<PlacedFeature> key, ResourceKey<ConfiguredFeature<?, ?>> feature, PlacementModifier... placementModifiers) {
        register(context, key, feature, List.of(placementModifiers));
    }
     ...
     */
    public static void register(BootstrapContext<PlacedFeature> context, ResourceKey<PlacedFeature> key, ResourceKey<ConfiguredFeature<?, ?>> feature, List<PlacementModifier> placementModifiers) {
        Holder<ConfiguredFeature<?, ?>> holder = context.lookup(Registries.CONFIGURED_FEATURE).getOrThrow(feature);
        PlacementUtils.register(context, key, holder, placementModifiers);
    }
